...
[1b]绝对节拍时间 //将在音频文件的00:00.700时刻使用cursor0输出"绝对节拍时间" *注：所有用到了节拍时间的[timeline]必须在使用了[bpm]以后才能使用
...
[square x1,y1,x2,y2,char]//在x1，y1到x2,y2的每一个像素上都输出char。**注意**：1、如果char是全角字符（如中文、全角标点、Nerd Font、Emoji等），将会自动间隔输出；2、可以用[space]表示空格
